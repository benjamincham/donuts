/**
 * Knowledge Base Storage Construct
 * Provides S3 bucket for knowledge base documents
 */
...
export interface KnowledgeBaseStorageProps {
   ...
   * Actual bucket name: {prefix}-kb-storage-{account}-{region}
   */
...
/**
 * Knowledge Base Storage Construct
 * Provides S3 bucket for knowledge base document storage
 */
export class KnowledgeBaseStorage extends Construct {
  ...
  constructor(scope: Construct, id: string, props?: KnowledgeBaseStorageProps) {
    ...
    this.bucket = new s3.Bucket(this, 'KnowledgeBaseStorageBucket', {
      bucketName: `${prefix}-kb-storage-${stack.account}-${stack.region}`,
      // Security settings
    ...
    // Add tags
    cdk.Tags.of(this.bucket).add('Component', 'KnowledgeBaseStorage');
  ...
  /**
   * Grant permission to put objects (for document uploads)
   */
  public grantPutAccess(grantee: iam.IGrantable): iam.Grant {
   ...
   */
  public grantDeleteAccess(grantee: iam.IGrantable): iam.Grant {
